...
function sendCurrencySetting() {
	...
	var ltcBid = $('#ltcInput').val();
	var bchBid = $('#bchInput').val();
	if (!$.isNumeric(btcBid ) || !$.isNumeric(ethBid) || !$.isNumeric(etcBid)|| !$.isNumeric(ltcBid) || !$.isNumeric(bchBid)) {
		alert('숫자를 입력하세요~');
		return false;
	}
	$.ajax({
